...
public class UserProblemServiceImlp implements UserProblemService {
    ...
    public Page<UserProblemResponse> getUserProblemsByUserId(int id, Pageable pageable) {
        ...
        if(user!=null){
            Page<UserProblem> userProblems = userProblemsRepository.findAllByUserId((long) id, pageable);
    ...
    @Override
    public SummaryResponseDto countSummeryOfUser(int id) {
        Users user = jwtUtil.getLoggedInUser();
        SummaryResponseDto summaryResponseDto = new SummaryResponseDto(0L,0L,0.0);
        if(user!=null){
