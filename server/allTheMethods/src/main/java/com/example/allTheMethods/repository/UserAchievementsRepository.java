...
public interface UserAchievementsRepository extends JpaRepository<UserAchievements, Long> {
    ...
    @Query("SELECT up.problem.category, COUNT(up) FROM UserProblem up WHERE up.user.id = :userId GROUP BY up.problem.category")
    Object[] countProblemsByCategory(@Param("userId") Long userId);
}
